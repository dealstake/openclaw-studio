...
export const ThinkingBlock = React.memo(function ThinkingBlock({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className={className}>
      <CollapsibleTrigger aria-label={streaming ? "Thinking in progress" : "Show agent thought process"} className="group/thinking flex items-center gap-1.5 rounded-md px-3 py-3 min-h-[44px] sm:px-2 sm:py-1.5 sm:min-h-[36px] text-left transition-colors hover:bg-muted/50">
        {/* Chevron */}
        <ChevronRight
          size={14}
          strokeWidth={1.75}
