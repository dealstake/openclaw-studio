/**
 * Bash+Python scripts for trashing and restoring agent state directories.
 * Used by the gateway/agent-state API route via SSH execution.
 *
 * These scripts run on the gateway host and move agent directories
 * to/from a timestamped trash folder under ~/.openclaw/trash/.
 */

/** Moves agent dirs (workspace + brain) to a timestamped trash folder. */
export const TRASH_SCRIPT = `
set -euo pipefail
...
/** Restores agent dirs from a trash folder back to their original locations. */
