...
function rowToBaseline(row: typeof agentBaselines.$inferSelect): AgentBaseline {
  ...
  return {
    ...
    toolErrorRate: toStats(row.toolErrorRateMean ?? 0, row.toolErrorRateStdDev ?? 0, row.toolErrorRateSampleCount ?? 0),
...
export function upsertBaseline(db: StudioDb, baseline: AgentBaseline): void {
  db.insert(agentBaselines)
    .values({
      ...
      toolErrorRateMean: baseline.toolErrorRate.mean,
      toolErrorRateStdDev: baseline.toolErrorRate.stdDev,
      toolErrorRateSampleCount: baseline.toolErrorRate.sampleCount,
