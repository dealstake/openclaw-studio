...
- **Auth UI config** (`src/lib/auth/sign-in-methods.ts`): reads `NEXT_PUBLIC_SSO_*` environment variables to determine which sign-in methods (Google, Microsoft, email) are enabled on the login page. Cloudflare Access handles actual authentication; this module only controls the login page button visibility and disabled states. Login (`src/app/login/page.tsx`) and logout (`src/app/logout/page.tsx`) are branded pages that use `src/components/brand/` components and follow the same Tailwind + CSS custom property patterns as the rest of the app.
